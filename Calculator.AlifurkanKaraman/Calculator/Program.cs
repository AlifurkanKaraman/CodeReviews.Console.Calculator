...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (!endApp)
        {
            ...
            {
                case "1":
                    double cleanNum1 = GetNumberFromUser(calculator, "Type a number for the first number, or enter 'p' to use a previous result: ");
                    double cleanNum2 = GetNumberFromUser(calculator, "Type a number for the second number, enter 'p' to use a previous result or press enter for single calculations: ");
        ...
        calculator.Finish();
        Console.WriteLine($"Total calculator usage: {calculator.GetUsageCount()} times.");
    }
    ...
    static double GetNumberFromUser(Calculator calculator, string prompt)
    {
        ...
        else if (string.IsNullOrEmpty(input) && !prompt.Contains("first"))
        {
            return double.NaN;
        }
